  ...
  <view>
    <view class="sunsin_picture_list">
      <view v-for="(item,index) in upload_picture_list" :key="index" class="sunsin_picture_item">
        ...
        <image v-show="item.upload_percent == 100" class="sunsin_width" :data-src="item.path_server"
               :src="item.path_server"
               mode="aspectFill" :data-idx="index" @click="previewImgs"></image>
        <view class="sunsin_upload_progress sunsin_width" v-show="item.upload_percent < 100" :data-index="index"
              @click="previewImg">{{item.upload_percent}}%
        </view>
      ...
      <view>
        <view class='sunsin_picture_item sunsin_width'
          ...
          <view class="sunsin_add_image sunsin_width" @click='chooseImage(upImgConfig.count)'
            ...
            <view>
              <text class="iconfont"
                    :class="[upImgConfig.upIconName==undefined?'icon-shangchuantupian':upImgConfig.upIconName]"
                    style="font-size: 72upx;margin-bottom: 12upx;"
                    :style="'color:'+upImgConfig.upIconColor+';width:100%;'"></text>
            </view>
...
<script>
    ...
    const upload_task = await uni.uploadFile({
      ...
      async success(res) {
        if (res.statusCode == 200) {
          ...
          console.log(` 后端上传( 成功返回 : `, data);
          if (data.code != 0) {
            return false;
          }
          console.log(`%c 后端上传(成功返回地址):${data.data.src}`, 'color:#1AAD19');
          // 先获取图片url(各个后端返回值不一，所以造成出入)
          // 修改获取的图片返回值路径
          // 提示：data.info改为你图片返回地址即可
          upload_picture_list[j]['path_server'] = data.data.src; //修改返回图片地址即可(2019/05/06) => 无论是腾讯云还是阿里云或者其它(只要返回图片地址以及引入对应的js文件进行一些配置即可扩展本插件)
  ...
  const tImage = async (_this, upload_picture_list, count) => {
    await _this.$emit('onUpImg', {file_list: _this.upload_picture_list, datakey: _this.datakey});
  }
