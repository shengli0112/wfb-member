<template>
	<view class="content">
		...
		<view class="orderBox">
			...
			<view class="orderList" v-for="(val,index) in orderList" :key='index' @click="navToDetail(val)">
				<view class="listTop">
					<view class="topLeft">
						<view class="userImg"><image src="../../static/image/user2x.png" mode=""></image></view>
...
<script>
	...
	export default {
      ...
      this.getData(this.tabCurrentIndex)
        ...
        console.log(val)
				uni.navigateTo({
					url: '/pages/user/reportHouseDetail?detail='+JSON.stringify(val)
				})
			},
			getData(status) {
				this.$api.httpRequest(
					'GET',
					this.$constant.API_HOME + '/report/report/my/page',
					{
						'page': this.page,
						'limit': 10,
						'status':status
