<template>
	<view class="content">
		<view class="orderBox">
			<view class="msg-img">
				<textarea name="" id="" placeholder="请输入意见描述" v-model="userNote" maxlength='-1'></textarea>
				<view>
					<view class="msg">
						<text>手机号</text>
						<input type="text" value="" placeholder="请填写您的手机号码(选填)" v-model="phNumber"/>
					</view>
					<view class="imgBox">
						<view class="imgTitle">上传截图（选填）</view>
			...
			submit(){
				...
				// return false;
					this.$api.httpRequest(
						'POST',
						this.$constant.API_HOME + 'feedback/feedback' , 
					 {
							description:this.userNote,
							contactInfo:this.phNumber,
							pictures:this.basicArr
						},
						{
