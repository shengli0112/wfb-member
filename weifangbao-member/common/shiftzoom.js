var cvi_szclback      = null;
var cvi_sztimer       = null;
var cvi_szactive      = null;
var cvi_szimage       = null;
var _touchStartPoints = [];
var shiftzoom         = {
  _shiftzoom: null, version: 3.9, released: '2019-09-04', defaultFading: true, defaultButtons: true,
  defaultOverview: true, defaultNozoom: false, defaultWheelinvert: true, defaultWheelstep: 20, defaultIcons: null,
  defaultBicubic: false, defaultForcetouchui: false, defaultShowcoords: false, defaultPixelcoords: true,
  defaultRelativecoords: false, defaultPercentcoords: false, defaultLowres: '', defaultMillisec: 40,
  defaultOpacity: 90,
  defaultOvsfact: 25, defaultOvaopac: 75, defaultOvacolor: 'red', defaultOvbcolor: 'white', defaultOvborder: '',
  defaultCurpath: '', defaultZoom: 0, defaultXpos: 50, defaultYpos: 50,
  ...
  add: function(ele, opts){
    ...
    if(!ele.active){
      ...
      var defopts          = {
        "curpath": shiftzoom.defaultCurpath, "opacity": shiftzoom.defaultOpacity,
        "millisec": shiftzoom.defaultMillisec,
        "intitle": shiftzoom.defaultIntitle, "outtitle": shiftzoom.defaultOuttitle,
        "infoblock": shiftzoom.defaultInfoblock, "wheelinvert": shiftzoom.defaultWheelinvert,
        "wheelstep": shiftzoom.defaultWheelstep, "ovsfact": shiftzoom.defaultOvsfact,
        "ovaopac": shiftzoom.defaultOvaopac, "ovacolor": shiftzoom.defaultOvacolor,
        "ovbcolor": shiftzoom.defaultOvbcolor, "zoom": shiftzoom.defaultZoom, "xpos": shiftzoom.defaultXpos,
        "ypos": shiftzoom.defaultYpos, "lowres": shiftzoom.defaultLowres, "icons": shiftzoom.defaultIcons,
        "bicubic": shiftzoom.defaultBicubic, "forcetouchui": shiftzoom.defaultForcetouchui
      };
      if(opts){
      ...
      if(document.images && document.createElement && document.getElementById && document.getElementsByTagName){
        ...
        var st, tmp, over, view, div = ele.parentNode, img = shiftzoom.E('div'), xref = shiftzoom.E('img'),
            outer                                                                     = shiftzoom.E('div'), map, mpn                                      = ele.useMap.split("#");
        ...
        img.opts    = defopts;
        img.highres = ele.src;
        console.log('ele.naturalWidth = ', ele.naturalWidth);
        ...
        if(ele.width >= 100 && ele.width <= img.maxwidth && ele.height >= 100 && ele.height <= img.maxheight){
          ...
          if(img.tod){
            ...
            // img.ondblclick    = function(){shiftzoom._setCursor(this, 1, img.id, true)};
  ...
  kenburns: function(img, obj){
    if(img && typeof (img.ctrlid) === "string"){
      ...
      var io = false, x = obj[0], y = obj[1], z = obj[2], p = obj[3] || false, s = obj[4] || false, d = obj[5] || false,
          b                                                                                           = obj[6] || false, a                                                                      = obj[7] || false;
      if(typeof (p) === "number"){
        if(p == 3){
          io = true
        }else if(p == 0){
        ...
        if(c > t){
          ...
          if(img.automode){
            if(b && typeof (b) === "string" && eval('typeof ' + b) == "function"){
              window.setTimeout(b + "('" + (typeof (a) == 'number' || typeof (a) == 'string' ? a : '') + "')", 30)
            }
            window.setTimeout("shiftzoom._next(shiftzoom.G('" + img.id + "'))", img.autowait + 1000)
          }else{
            if(b && typeof (b) === "string" && eval('typeof ' + b) == "function"){
              window.setTimeout(b + "('" + (typeof (a) == 'number' || typeof (a) == 'string' ? a : '') + "')", 200)
            }
          }
        }
      }, d)
  ...
  _showCoords: function(e){
    if(cvi_szactive != null){
      var k, t, x, y, ex, ey, px = 0, py = 0, o = shiftzoom.G(cvi_szactive), w = o.parentNode.width,
  ...
  _showPixel: function(e){
    if(cvi_szactive != null){
      var k, t, x, y, z, ex, ey, px = 0, py = 0, o = shiftzoom.G(cvi_szactive), w = o.parentNode.width,
          h                                                                       = o.parentNode.height;
      e                                                                           = e ? e : window.event;
      ex                                                                          = e.clientX;
      ey                                                                          = e.clientY;
  ...
  _showPercent: function(e){
    if(cvi_szactive != null){
      var k, t, x, y, z, ex, ey, px = 0, py = 0, na = !Number.prototype.toFixed ? 0 : 1,
          o                                         = shiftzoom.G(cvi_szactive),
          w                                         = o.parentNode.width, h                 = o.parentNode.height;
      e                                             = e ? e : window.event;
      ex                                            = e.clientX;
      ey                                            = e.clientY;
      if(e.pageX || e.pageY){
  ...
  _showLatLon: function(e){
    if(cvi_szactive != null){
      var k, t, x, y, lat, lon, ex, ey, px = 0, py = 0, o = shiftzoom.G(cvi_szactive), w = o.parentNode.width,
          h                                                                              = o.parentNode.height;
  ...
  _showTooltip: function(id, xx, yy){
    xx                  = xx || 0;
    yy                  = yy || 0;
    var over, view, img = shiftzoom.G(id), ison = shiftzoom.G(img.ttipid);
    if(!ison){
      ...
      over.onclick = function(){document.getElementsByTagName("body")[0].removeChild(this)};
      if(!img.trident){
        view                       = shiftzoom.E('div');
  ...
  _stopZoom: function(){
    ...
    cvi_sztimer          = setInterval("shiftzoomFadeInfo('" + img.id + "',100)", 30);
  ...
  _catchKey: function(e){
    ...
    if(((e.altKey && !e.shiftKey) || rm || img.zoomout) && !img.automode && !img.nozoom && (img.parentNode.width > img.minwidth || img.parentNode.height > img.minheight)){
      ...
      cvi_sztimer = setInterval("shiftzoom._zoomOut('" + img.id + "'," + +"," + (img.webkit && rm ? 1 : 0) + "," + st + "," + sw + "," + ew + "," + sh + "," + eh + "," + sx + "," + ex + "," + sy + "," + ey + "," + img.nozoom + ")", img.millisec)
      ...
      var x                = parseInt(img.parentNode.style.left, 10), y = parseInt(img.parentNode.style.top, 10);
  ...
  _catchTouch: function(e){
    document.getElementById('log_wrap').innerText = '_catchTouch ' + e.touches.length
    ...
    if(e.touches.length == 1){
      ...
      if(img && typeof (img.ctrlid) === "string"){
        ...
        if(delay < 200 && delay > 0){ // 双击时间间隔
          ...
          var k, t, ex, ey, px = 0, py = 0, obj = shiftzoom.G(img.infoid).style;
          obj.opacity          = 1;
          obj.visibility       = 'hidden';
    ...
    if(e.touches.length == 2){
      _touchStartPoints = e.touches;

      // document.getElementById('log_wrap').prepend(`_catchTouch  _touchStartPoints[0].pageX =` + e.touches[0].pageX +"\n" );
    }
    return false
  },
  _touchMove: function(e){
    document.getElementById('log_wrap').innerText = '_touchMove ' + e.touches.length
    ...
    if(e.touches.length == 2){
      ...
      let scale = getDistance(e.touches[0], e.touches[1]) - getDistance(_touchStartPoints[0], _touchStartPoints[1]);
      // document.getElementById('log_wrap').prepend(`_touchMove 2  scale = ${scale} <br>` );
      // wlogger('scale = '+ scale);
      if(e.preventDefault){
        e.preventDefault()
      }else{
      ...
      // wlogger('typeof _zoomHelper = ' + typeof _zoomHelper);
      _zoomHelper(scale > 0 ? 1 : -1);
    }
    return false
  },
  _touchEnd: function(){
  ...
  _gestureChange: function(e){
    ...
    var img = shiftzoom._shiftzoom;
    if(img && typeof (img.ctrlid) === "string"){
      var n, obj = shiftzoom.G(img.infoid).style, p = shiftzoom.get(img, 'currentxyz');
      if(p.z <= 0){
        p.x = 50;
        p.y = 50
      }
  ...
  _fadeImage: function(id, o){
    ...
    if(o <= 100){
      ...
      o += 10;
      // window.setTimeout("this._fadeImage('" + id + "'," + o + ")", 30)
      setTimeout(this._fadeImage(id, o), 30)
    }else{
      if(img.buttons && !img.tod){
        shiftzoom.G(img.ctrlid).style.visibility = 'visible'
      }
...
var getDistance = function(p1, p2){
  document.getElementById('log_wrap').prepend(' _getDistance  p1.pageX= ' + p1.pageX);
  document.getElementById('log_wrap').prepend(' _getDistance  p1.pageY= ' + p1.pageY);
  let x = p2.pageX.toFixed(2) - p1.pageX.toFixed(2);
  let y = p2.pageY.toFixed(2) - p1.pageY.toFixed(2);
 ...
 * log
 */
var wlogger = function(loginfo, isAppend = false, level = 'log'){
  const WRAPID    = 'log_wrap';
  const LOG_COLOR = {log: '#333', war: '#ffac31', error: '#ff505c'};
  let color       = LOG_COLOR[level];
  let logDom      = `<span style="color:${color}">[ ${level} ] ${loginfo}</span> <br>`;
  if(isAppend){
    document.getElementById(WRAPID).appendChild(logDom);
  }else{
    document.getElementById(WRAPID).innerHTML = logDom;
  }
}
 ...
 * v =-1 缩小
 * */
var _zoomHelper = function(v){
  if(!cvi_szactive || !v){
    return false;
  }
  cvi_szimage = true;

  var img = shiftzoom.G(cvi_szactive), obj = shiftzoom.G(img.infoid).style;
  if(!img.nozoom){
    let d          = img.invert ? (v < 0 ? 0 : 1) : (v < 0 ? 1 : 0);
    obj.visibility = 'visible';
    obj.opacity    = 1;
    let p          = shiftzoom.get(img, 'currentxyz');
    if(p.z <= 0){
      p.x = 50;
      p.y = 50
    }
    ...
    let n = d < 1 ? Math.max(p.z - img.speed, 0) : Math.min(p.z + img.speed, 100);
